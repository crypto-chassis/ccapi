...
#include "ccapi_cpp/service/ccapi_market_data_service_huobi_usdt_perpetual.h"
#endif
...
#include "ccapi_cpp/service/ccapi_execution_management_service_huobi_usdt_perpetual.h"
...
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_OKEX_PERPETUAL
#include "ccapi_cpp/service/ccapi_execution_management_service_okex_perpetual.h"
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_KUCOIN
...
namespace ccapi {
    ...
    public:
        ...
        Session(const SessionOptions &sessionOptions = SessionOptions(),
                const SessionConfigs &sessionConfigs = SessionConfigs(),
        ...
        void start() {
            ...
            this->t = std::move(t);
            std::function<void(Event &event)> eventHandler = std::bind(&Session::onEvent, this, std::placeholders::_1,
                                                                       &eventQueue);
#ifdef CCAPI_ENABLE_SERVICE_MARKET_DATA
#ifdef CCAPI_ENABLE_EXCHANGE_COINBASE
...
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_HUOBI_USDT_PERPETUAL
              this->serviceByServiceNameExchangeMap[CCAPI_MARKET_DATA][CCAPI_EXCHANGE_NAME_HUOBI_USDT_PERPETUAL] = std::make_shared<MarketDataServiceHuobiUsdtPerpetual>(eventHandler, sessionOptions, sessionConfigs, this->serviceContextPtr);
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_OKEX
...
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_HUOBI_USDT_PERPETUAL
            this->serviceByServiceNameExchangeMap[CCAPI_EXECUTION_MANAGEMENT][CCAPI_EXCHANGE_NAME_HUOBI_USDT_PERPETUAL] = std::make_shared<ExecutionManagementServiceHuobiUsdtPerpetual>(
                    eventHandler, sessionOptions, sessionConfigs, this->serviceContextPtr);
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_ERISX
...
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_OKEX_PERPETUAL
            this->serviceByServiceNameExchangeMap[CCAPI_EXECUTION_MANAGEMENT][CCAPI_EXCHANGE_NAME_OKEX_PERPETUAL] = std::make_shared<ExecutionManagementServiceOkexPerpetual>(eventHandler, sessionOptions, sessionConfigs, this->serviceContextPtr);
#endif
#ifdef CCAPI_ENABLE_EXCHANGE_KUCOIN
        ...
        void subscribe(const std::vector<Subscription> &subscriptionList) {
            ...
            for (const auto &x : subscriptionListByServiceNameMap) {
                ...
                if (this->serviceByServiceNameExchangeMap.find(serviceName) ==
                    this->serviceByServiceNameExchangeMap.end()) {
                ...
                if (serviceName == CCAPI_MARKET_DATA) {
                    ...
                    for (const auto &subscription : subscriptionList) {
                        ...
                        if (exchangeFieldMap.find(exchange) == exchangeFieldMap.end()
                            || std::find(exchangeFieldMap.find(exchange)->second.begin(),
                        ...
                        if (field == CCAPI_MARKET_DEPTH) {
                            ...
                            if (((exchange == CCAPI_EXCHANGE_NAME_KRAKEN || exchange == CCAPI_EXCHANGE_NAME_BITSTAMP ||
                                  exchange == CCAPI_EXCHANGE_NAME_BITFINEX || exchange == CCAPI_EXCHANGE_NAME_HUOBI ||
                                  exchange == CCAPI_EXCHANGE_NAME_HUOBI_USDT_PERPETUAL ||
                                  exchange == CCAPI_EXCHANGE_NAME_OKEX ||
                                  exchange == CCAPI_EXCHANGE_NAME_OKEX_PERPETUAL)
...
//            }
                    ...
                    if (!unsupportedExchangeMarketDepthSet.empty()) {
                        this->onError(Event::Type::SUBSCRIPTION_STATUS, Message::Type::SUBSCRIPTION_FAILURE,
                                      "unsupported exchange market depth: " +
                                      toString(unsupportedExchangeMarketDepthSet)
                    ...
                    for (auto &subscriptionListByExchange : subscriptionListByExchangeMap) {
                        ...
                        std::map<std::string, wspp::lib::shared_ptr<Service> > &serviceByExchangeMap = this->serviceByServiceNameExchangeMap.at(
                                serviceName);
        ...
        void sendRequest(const std::vector<Request> &requestList, Queue<Event> *eventQueuePtr = nullptr) {
            ...
            for (const auto &request : requestList) {
                ...
                if (eventQueuePtr && serviceNameExchangeSet.find(key) == serviceNameExchangeSet.end()) {
                    servicePtr->setEventHandler(
                            std::bind(&Session::onEvent, this, std::placeholders::_1, eventQueuePtr));
