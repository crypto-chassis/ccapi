...
//  resolver(*serviceContextPtr->ioContextPtr),
        ...
        sendRequest(const Request &request, const bool useFuture, const TimePoint &now) override {
    ...
    protected:
        ...
        void performRequest(std::shared_ptr<HttpConnection> httpConnectionPtr, const Request &request,
            ...
            beast::get_lowest_layer(stream).async_connect(
                    ...
                    beast::bind_front_handler(&ExecutionManagementService::onConnect,
        ...
        void onConnect(std::shared_ptr<HttpConnection> httpConnectionPtr, Request request,
            ...
            stream.async_handshake(
                    ...
                    beast::bind_front_handler(&ExecutionManagementService::onHandshake,
                                              shared_from_base<ExecutionManagementService>(), httpConnectionPtr,
                                              request, req, retry));
            CCAPI_LOGGER_TRACE("after async_handshake");
        }

        void onHandshake(std::shared_ptr<HttpConnection> httpConnectionPtr, Request request,
                         http::request<http::string_body> req, HttpRetry retry, beast::error_code ec) {
            ...
            std::shared_ptr<http::request<http::string_body> > reqPtr(
                    new http::request<http::string_body>(std::move(req)));
            CCAPI_LOGGER_TRACE("before async_write");
            http::async_write(stream, *reqPtr, beast::bind_front_handler(&ExecutionManagementService::onWrite,
                                                                         shared_from_base<ExecutionManagementService>(),
                                                                         httpConnectionPtr, request, reqPtr, retry));
            CCAPI_LOGGER_TRACE("after async_write");
        }

        void onWrite(std::shared_ptr<HttpConnection> httpConnectionPtr, Request request,
            ...
            http::async_read(stream, *bufferPtr, *resPtr, beast::bind_front_handler(&ExecutionManagementService::onRead,
                                                                                    shared_from_base<ExecutionManagementService>(),
                                                                                    httpConnectionPtr, request, reqPtr,
                                                                                    retry, bufferPtr, resPtr));
            CCAPI_LOGGER_TRACE("after async_read");
        }

        void onRead(std::shared_ptr<HttpConnection> httpConnectionPtr, Request request,
                    std::shared_ptr<http::request<http::string_body> > reqPtr, HttpRetry retry,
                    std::shared_ptr<beast::flat_buffer> bufferPtr,
                    std::shared_ptr<http::response<http::string_body> > resPtr, beast::error_code ec,
                    std::size_t bytes_transferred) {
            ...
            if (retry.numRetry <= this->sessionOptions.httpMaxNumRetry &&
                ...
                try {
                    if (this->sessionOptions.enableOneHttpConnectionPerRequest || this->httpConnectionPool.empty()) {
                        ...
                        try {
                            streamPtr = this->createStream(this->serviceContextPtr->ioContextPtr,
                                                           this->serviceContextPtr->sslContextPtr, this->host);
                        } catch (const beast::error_code &ec) {
                            CCAPI_LOGGER_TRACE("fail");
                            this->onError(Event::Type::REQUEST_STATUS, Message::Type::REQUEST_FAILURE, ec,
                                          "create stream");
                            return;
                        }
                        std::shared_ptr<HttpConnection> httpConnectionPtr(
                                new HttpConnection(this->host, this->port, streamPtr));
                ...
                std::string errorMessage = this->sessionOptions.httpMaxNumRetry ? "max retry exceeded"
...
//  }
        ...
        processSuccessfulTextMessage(const Request &request, const std::string &textMessage,
        ...
        virtual Element extractOrderInfo(const rj::Value &x,
                ...
                if (it != x.MemberEnd()) {
                    std::string value = y.second.second == JsonDataType::STRING ? it->value.GetString() :
                                        y.second.second == JsonDataType::INTEGER ? std::to_string(it->value.GetInt64())
                                                                                 :
                                        y.second.second == JsonDataType::BOOLEAN ? std::to_string(
                    ...
                    if (y.first == CCAPI_EM_ORDER_INSTRUMENT) {
                        ...
                        value = UtilString::toLower(value).rfind("buy", 0) == 0 ? CCAPI_EM_ORDER_SIDE_BUY
        ...
        virtual void convertReq(const Request &request, const TimePoint &now, http::request<http::string_body> &req,
                                const std::map<std::string, std::string> &credential, const std::string &symbolId,
                                const Request::Operation operation) = 0;

        virtual std::vector<Element>
        extractOrderInfo(const Request::Operation operation, const rj::Document &document) = 0;
