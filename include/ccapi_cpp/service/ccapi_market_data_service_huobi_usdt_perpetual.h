#ifndef EXAMPLE_CCAPI_MARKET_DATA_SERVICE_HUOBI_USDT_PERPETUAL_H
#define EXAMPLE_CCAPI_MARKET_DATA_SERVICE_HUOBI_USDT_PERPETUAL_H
#ifdef CCAPI_ENABLE_SERVICE_MARKET_DATA
#ifdef CCAPI_ENABLE_EXCHANGE_HUOBI_USDT_PERPETUAL
#include "ccapi_cpp/service/ccapi_market_data_service.h"
#include <regex>
namespace ccapi {
    class MarketDataServiceHuobiUsdtPerpetual CCAPI_FINAL : public MarketDataService {
    public:
        MarketDataServiceHuobiUsdtPerpetual(std::function<void(Event& event)> wsEventHandler, SessionOptions sessionOptions, SessionConfigs sessionConfigs, std::shared_ptr<ServiceContext> serviceContextPtr): MarketDataService(wsEventHandler, sessionOptions, sessionConfigs, serviceContextPtr) {
        ...
        std::vector<std::string> createRequestStringList(const WsConnection& wsConnection) override {
            ...
            for (const auto & subscriptionListByChannelIdSymbolId : this->subscriptionListByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)) {
                ...
                for (auto & subscriptionListByInstrument : subscriptionListByChannelIdSymbolId.second) {
                    ...
                    if (channelId.rfind(CCAPI_WEBSOCKET_HUOBI_USDT_PERPETUAL_CHANNEL_MARKET_DEPTH, 0) == 0) {
                        this->l2UpdateIsReplaceByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId] = true;
                        exchangeSubscriptionId = CCAPI_WEBSOCKET_HUOBI_USDT_PERPETUAL_CHANNEL_MARKET_DEPTH;
                    } else if (channelId.rfind(CCAPI_WEBSOCKET_HUOBI_USDT_PERPETUAL_CHANNEL_TRADE_DETAIL, 0) == 0) {
                        exchangeSubscriptionId = CCAPI_WEBSOCKET_HUOBI_USDT_PERPETUAL_CHANNEL_TRADE_DETAIL;
                    }
...
//            }
        ...
        std::vector<MarketDataMessage> processTextMessage(wspp::connection_hdl hdl, const std::string& textMessage, const TimePoint& timeReceived) override {
            ...
            if (document.IsObject() && document.HasMember("ch") && document.HasMember("tick")) {
                ...
                if (std::regex_search(channelId, std::regex(CCAPI_WEBSOCKET_HUOBI_USDT_PERPETUAL_CHANNEL_MARKET_DEPTH_REGEX))) {
                ...
                } else if (std::regex_search(channelId, std::regex(CCAPI_WEBSOCKET_HUOBI_USDT_PERPETUAL_CHANNEL_TRADE_DETAIL_REGEX))) {
                    const rj::Value& tick = document["tick"];
                    std::string ts = tick["ts"].GetString();
                    ts.insert(ts.size() - 3, ".");
...
#endif
#endif
#endif //EXAMPLE_CCAPI_MARKET_DATA_SERVICE_HUOBI_USDT_PERPETUAL_H
