#ifndef EXAMPLE_CCAPI_MARKET_DATA_SERVICE_OKEX_PERPETUAL_H
#define EXAMPLE_CCAPI_MARKET_DATA_SERVICE_OKEX_PERPETUAL_H
#ifdef CCAPI_ENABLE_SERVICE_MARKET_DATA
#ifdef CCAPI_ENABLE_EXCHANGE_OKEX_PERPETUAL
#include "ccapi_cpp/service/ccapi_market_data_service.h"
namespace ccapi {
class MarketDataServiceOkexPerpetual CCAPI_FINAL : public MarketDataService {
 public:
  MarketDataServiceOkexPerpetual(std::function<void(Event& event)> wsEventHandler, SessionOptions sessionOptions, SessionConfigs sessionConfigs, std::shared_ptr<ServiceContext> serviceContextPtr): MarketDataService(wsEventHandler, sessionOptions, sessionConfigs, serviceContextPtr) {
...
//    std::cout << textMessage.c_str() << std::endl;
    ...
    if (document.IsObject() && document.HasMember("table")) {
      ...
      if (table == CCAPI_WEBSOCKET_OKEX_PERPETUAL_CHANNEL_PUBLIC_DEPTH400) {
      ...
      } else if (table == CCAPI_WEBSOCKET_OKEX_PERPETUAL_CHANNEL_TRADE) {
        for (const auto& datum : document["data"].GetArray()) {
          std::string exchangeSubscriptionId = table + ":" + datum["instrument_id"].GetString();
          MarketDataMessage wsMessage;
          wsMessage.type = MarketDataMessage::Type::MARKET_DATA_EVENTS;
...
#endif
#endif
#endif //EXAMPLE_CCAPI_MARKET_DATA_SERVICE_OKEX_PERPETUAL_H
