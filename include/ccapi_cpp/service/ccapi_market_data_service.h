...
class MarketDataService : public Service {
 public:
  ...
  MarketDataService(std::function<void(Event& event)> eventHandler, SessionOptions sessionOptions,
    ...
    this->pingIntervalMilliSecondsByMethodMap[PingPongMethod::WEBSOCKET_PROTOCOL_LEVEL] =
        sessionOptions.pingIntervalMilliSeconds;
    this->pongTimeoutMilliSecondsByMethodMap[PingPongMethod::WEBSOCKET_PROTOCOL_LEVEL] =
        sessionOptions.pongTimeoutMilliSeconds;
    ...
    if (this->shouldContinue.load()) {
      for (const auto& x : this->groupSubscriptionListByInstrumentGroup(subscriptionList)) {
        ...
        wspp::lib::asio::post(
            ...
            [that = shared_from_base<MarketDataService>(), instrumentGroup, subscriptionListGivenInstrumentGroup]() {
              ...
              if (wsConnectionIdListByInstrumentGroupMap.find(instrumentGroup) !=
                      wsConnectionIdListByInstrumentGroupMap.end() &&
                  that->subscriptionStatusByInstrumentGroupInstrumentMap.find(instrumentGroup) !=
                      that->subscriptionStatusByInstrumentGroupInstrumentMap.end()) {
                ...
                for (const auto& subscription : subscriptionListGivenInstrumentGroup) {
                  ...
                  if (that->subscriptionStatusByInstrumentGroupInstrumentMap[instrumentGroup].find(instrument) !=
                    ...
                    that->onError(Event::Type::SUBSCRIPTION_STATUS, Message::Type::SUBSCRIPTION_FAILURE,
                                  "already subscribed: " + toString(subscription));
                    return;
                  }
                  wsConnection.subscriptionList.push_back(subscription);
                  that->subscriptionStatusByInstrumentGroupInstrumentMap[instrumentGroup][instrument] =
 ...
 protected:
  ...
  std::map<std::string, std::vector<Subscription>> groupSubscriptionListByInstrumentGroup(
  ...
  virtual void onOpen(wspp::connection_hdl hdl) {
    ...
    if (this->enableCheckPingPongWebsocketProtocolLevel) {
      this->setPingPongTimer(PingPongMethod::WEBSOCKET_PROTOCOL_LEVEL, wsConnection, hdl,
                             [that = shared_from_base<MarketDataService>()](wspp::connection_hdl hdl, ErrorCode& ec) {
                               that->ping(hdl, "", ec);
                             });
    }
    ...
    for (const auto& subscription : wsConnection.subscriptionList) {
      auto instrument = subscription.getInstrument();
      this->subscriptionStatusByInstrumentGroupInstrumentMap[instrumentGroup][instrument] =
          Subscription::Status::SUBSCRIBING;
  ...
  std::string convertInstrumentToWebsocketSymbolId(std::string instrument) {
    ...
    if (!instrument.empty()) {
      if (this->sessionConfigs.getExchangeInstrumentSymbolMap().find(this->name) !=
  ...
  void prepareSubscription(const WsConnection& wsConnection, const Subscription& subscription) {
    ...
    if (field == CCAPI_MARKET_DEPTH) {
      if (this->name == CCAPI_EXCHANGE_NAME_KRAKEN || this->name == CCAPI_EXCHANGE_NAME_BITFINEX ||
          this->name == CCAPI_EXCHANGE_NAME_BINANCE_US || this->name == CCAPI_EXCHANGE_NAME_BINANCE ||
        ...
        marketDepthSubscribedToExchange = this->calculateMarketDepthSubscribedToExchange(
            marketDepthRequested, this->sessionConfigs.getWebsocketAvailableMarketDepth().at(this->name));
        ...
        if (marketDepthRequested <= 20) {
          channelId = std::string(CCAPI_WEBSOCKET_ERISX_CHANNEL_TOP_OF_BOOK_MARKET_DATA_SUBSCRIBE) + "?" +
                      CCAPI_MARKET_DEPTH_SUBSCRIBED_TO_EXCHANGE + "=" + std::to_string(marketDepthRequested);
          this->marketDepthSubscribedToExchangeByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId] =
    ...
    this->optionMapByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId].insert(optionMap.begin(),
                                                                                                   optionMap.end());
    ...
    CCAPI_LOGGER_TRACE("this->correlationIdListByConnectionIdChannelSymbolIdMap = " +
  ...
  void onFail_(WsConnection& wsConnection) {
    ...
    long seconds = std::round(UtilAlgorithm::exponentialBackoff(
    ...
    this->connectRetryOnFailTimerByConnectionIdMap[thisWsConnection.id] = this->serviceContextPtr->tlsClientPtr->set_timer(
        seconds * 1000, [thisWsConnection, that = shared_from_base<MarketDataService>(), urlBase](ErrorCode const& ec) {
          if (that->wsConnectionMap.find(thisWsConnection.id) == that->wsConnectionMap.end()) {
            if (ec) {
              CCAPI_LOGGER_ERROR("wsConnection = " + toString(thisWsConnection) +
                                 ", connect retry on fail timer error: " + ec.message());
              ...
              // that->instrumentGroupByWsConnectionIdMap.insert(std::pair<std::string, std::string>(thatWsConnection.id,
              // thatWsConnection.instrumentGroup));
  ...
  void clearStates(WsConnection& wsConnection) {
    ...
    if (this->pongTimeOutTimerByMethodByConnectionIdMap.find(wsConnection.id) !=
        this->pongTimeOutTimerByMethodByConnectionIdMap.end()) {
  ...
  virtual void onClose(wspp::connection_hdl hdl) {
    ...
    if (this->shouldContinue.load()) {
      ...
      // this->instrumentGroupByWsConnectionIdMap.insert(std::pair<std::string, std::string>(thisWsConnection.id,
      // thisWsConnection.instrumentGroup));
    }
    CCAPI_LOGGER_FUNCTION_EXIT;
  }
  void onMessage(wspp::connection_hdl hdl, TlsClient::message_ptr msg) {
    ...
    defined(CCAPI_ENABLE_EXCHANGE_OKEX)
      if (this->name == CCAPI_EXCHANGE_NAME_HUOBI || this->name == CCAPI_EXCHANGE_NAME_HUOBI_USDT_SWAP ||
          this->name == CCAPI_EXCHANGE_NAME_OKEX) {
        std::string decompressed;
        std::string payload = msg->get_payload();
        try {
          ErrorCode ec =
              this->inflater.decompress(reinterpret_cast<const uint8_t*>(&payload[0]), payload.size(), decompressed);
          if (ec) {
  ...
  void onPongByMethod(PingPongMethod method, wspp::connection_hdl hdl, const std::string& textMessage,
        ...
        if (wsMessage.type == MarketDataMessage::Type::MARKET_DATA_EVENTS) {
          if (wsMessage.recapType == MarketDataMessage::RecapType::NONE &&
            ...
            CCAPI_LOGGER_WARN("late websocket message: timeReceived = " + toString(timeReceived) +
                              ", wsMessage.tp = " + toString(wsMessage.tp) + ", wsConnection = " + toString(wsConnection));
          }
          event.setType(Event::Type::SUBSCRIPTION_DATA);
          std::string exchangeSubscriptionId = wsMessage.exchangeSubscriptionId;
          std::string channelId = this->channelIdSymbolIdByConnectionIdExchangeSubscriptionIdMap.at(wsConnection.id)
                                      .at(exchangeSubscriptionId)
                                      .at(CCAPI_CHANNEL_ID);
          std::string symbolId = this->channelIdSymbolIdByConnectionIdExchangeSubscriptionIdMap.at(wsConnection.id)
                                     .at(exchangeSubscriptionId)
                                     .at(CCAPI_SYMBOL_ID);
          auto field = this->fieldByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(channelId).at(symbolId);
          CCAPI_LOGGER_TRACE("this->optionMapByConnectionIdChannelIdSymbolIdMap = " +
          ...
          if (wsMessage.data.find(MarketDataMessage::DataType::BID) != wsMessage.data.end() ||
            ...
            if (this->processedInitialSnapshotByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId] &&
              ...
              this->processUpdateSnapshot(wsConnection, channelId, symbolId, event, shouldEmitEvent, wsMessage.tp,
                                          timeReceived, wsMessage.data, field, optionMap, correlationIdList, snapshotBid,
                                          snapshotAsk);
          ...
          if (wsMessage.data.find(MarketDataMessage::DataType::TRADE) != wsMessage.data.end()) {
            this->processTrade(wsConnection, channelId, symbolId, event, shouldEmitEvent, wsMessage.tp, timeReceived,
                               wsMessage.data, field, optionMap, correlationIdList);
          }
        } else {
  ...
  std::map<Decimal, std::string> calculateMarketDepthUpdate(bool isBid, const std::map<Decimal, std::string>& c1,
                                                            const std::map<Decimal, std::string>& c2, int maxMarketDepth) {
    if (c1.empty()) {
  ...
  void updateElementListWithUpdateMarketDepth(const std::string& field, const std::map<std::string, std::string>& optionMap,
    ...
    if (field == CCAPI_MARKET_DEPTH) {
      ...
      if (optionMap.at(CCAPI_MARKET_DEPTH_RETURN_UPDATE) == CCAPI_MARKET_DEPTH_RETURN_UPDATE_ENABLE) {
        ...
        const std::map<Decimal, std::string>& snapshotBidUpdate =
            this->calculateMarketDepthUpdate(true, snapshotBid, snapshotBidPrevious, maxMarketDepth);
        ...
        const std::map<Decimal, std::string>& snapshotAskUpdate =
            this->calculateMarketDepthUpdate(false, snapshotAsk, snapshotAskPrevious, maxMarketDepth);
  ...
  void updateElementListWithTrade(const std::string& field, const MarketDataMessage::TypeForData& input,
                                  std::vector<Element>& elementList) {
  ...
  void updateElementListWithOhlc(const WsConnection& wsConnection, const std::string& channelId, const std::string& symbolId,
    ...
    if (field == CCAPI_TRADE) {
      ...
      if (this->openByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId].empty()) {
        ...
        element.insert(CCAPI_HIGH,
                       this->highByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId].toString());
        element.insert(CCAPI_LOW,
                       this->lowByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId].toString());
            ...
            if (statusCode / 100 == 2) {
              ...
              try {
                ...
                for (const auto& subscription : thisWsConnection.subscriptionList) {
                  auto instrument = subscription.getInstrument();
                  that->subscriptionStatusByInstrumentGroupInstrumentMap[thisWsConnection.instrumentGroup][instrument] =
                      Subscription::Status::SUBSCRIBING;
                }
                ...
                CCAPI_LOGGER_TRACE("that->extraPropertyByConnectionIdMap = " +
                                   toString(that->extraPropertyByConnectionIdMap));
                return;
  ...
  void connect(WsConnection& wsConnection) {
    ...
    this->instrumentGroupByWsConnectionIdMap.insert(
        std::pair<std::string, std::string>(wsConnection.id, wsConnection.instrumentGroup));
    ...
    con->set_open_handler(
        std::bind(&MarketDataService::onOpen, shared_from_base<MarketDataService>(), std::placeholders::_1));
    con->set_fail_handler(
        std::bind(&MarketDataService::onFail, shared_from_base<MarketDataService>(), std::placeholders::_1));
    con->set_close_handler(
        std::bind(&MarketDataService::onClose, shared_from_base<MarketDataService>(), std::placeholders::_1));
    con->set_message_handler(std::bind(&MarketDataService::onMessage, shared_from_base<MarketDataService>(),
                                       std::placeholders::_1, std::placeholders::_2));
    if (this->sessionOptions.enableCheckPingPongWebsocketProtocolLevel) {
      con->set_pong_handler(std::bind(&MarketDataService::onPong, shared_from_base<MarketDataService>(),
                                      std::placeholders::_1, std::placeholders::_2));
    }
    con->set_ping_handler(std::bind(&MarketDataService::onPing, shared_from_base<MarketDataService>(), std::placeholders::_1,
                                    std::placeholders::_2));
  ...
  void processInitialSnapshot(const WsConnection& wsConnection, const std::string& channelId, const std::string& symbolId,
                              Event& event, bool& shouldEmitEvent, const TimePoint& tp, const TimePoint& timeReceived,
                              const MarketDataMessage::TypeForData& input, const std::string& field,
                              const std::map<std::string, std::string>& optionMap,
                              const std::vector<std::string>& correlationIdList, std::map<Decimal, std::string>& snapshotBid,
                              std::map<Decimal, std::string>& snapshotAsk) {
    snapshotBid.clear();
    snapshotAsk.clear();
    int maxMarketDepth = std::stoi(optionMap.at(CCAPI_MARKET_DEPTH_MAX));
    for (const auto& x : input) {
      ...
      if (type == MarketDataMessage::DataType::BID) {
        ...
        CCAPI_LOGGER_TRACE("lastNToString(snapshotBid, " + toString(maxMarketDepth) +
                           ") = " + lastNToString(snapshotBid, maxMarketDepth));
        ...
        CCAPI_LOGGER_TRACE("firstNToString(snapshotAsk, " + toString(maxMarketDepth) +
                           ") = " + firstNToString(snapshotAsk, maxMarketDepth));
      } else {
        CCAPI_LOGGER_WARN("extra type " + MarketDataMessage::dataTypeToString(type));
      }
    }
    ...
    if (shouldConflate) {
      ...
      CCAPI_LOGGER_TRACE(
          ...
          "symbolId) = " +
          toString(this->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                       .at(channelId)
                       .at(symbolId)));
      CCAPI_LOGGER_TRACE(
          "this->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(channelId).at("
          "symbolId) = " +
          toString(this->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                       .at(channelId)
                       .at(symbolId)));
  ...
  void processUpdateSnapshot(const WsConnection& wsConnection, const std::string& channelId, const std::string& symbolId,
                             Event& event, bool& shouldEmitEvent, const TimePoint& tp, const TimePoint& timeReceived,
                             const MarketDataMessage::TypeForData& input, const std::string& field,
                             const std::map<std::string, std::string>& optionMap,
                             const std::vector<std::string>& correlationIdList, std::map<Decimal, std::string>& snapshotBid,
    ...
    if (this->processedInitialSnapshotByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId]) {
      ...
      CCAPI_LOGGER_TRACE("firstNToString(snapshotAsk, " + toString(maxMarketDepth) +
                         ") = " + firstNToString(snapshotAsk, maxMarketDepth));
      CCAPI_LOGGER_TRACE("lastNToString(snapshotBidPrevious, " + toString(maxMarketDepth) +
                         ") = " + lastNToString(snapshotBidPrevious, maxMarketDepth));
      CCAPI_LOGGER_TRACE("firstNToString(snapshotAskPrevious, " + toString(maxMarketDepth) +
      ...
      CCAPI_LOGGER_TRACE("optionMap = " + toString(optionMap));
      bool shouldConflate =
          optionMap.at(CCAPI_CONFLATE_INTERVAL_MILLISECONDS) != CCAPI_CONFLATE_INTERVAL_MILLISECONDS_DEFAULT;
      ...
      bool intervalChanged =
          shouldConflate &&
          conflateTp >
              this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(channelId).at(symbolId);
      ...
      if (!shouldConflate || intervalChanged) {
        ...
        if (shouldConflate && intervalChanged) {
          const std::map<Decimal, std::string>& snapshotBidPreviousPrevious =
              this->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                  .at(channelId)
                  .at(symbolId);
          const std::map<Decimal, std::string>& snapshotAskPreviousPrevious =
              this->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                  .at(channelId)
                  .at(symbolId);
          this->updateElementListWithUpdateMarketDepth(field, optionMap, snapshotBidPrevious, snapshotBidPreviousPrevious,
                                                       snapshotAskPrevious, snapshotAskPreviousPrevious, elementList, false);
          this->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
              .at(channelId)
              .at(symbolId) = snapshotBidPrevious;
          this->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
              .at(channelId)
              .at(symbolId) = snapshotAskPrevious;
          CCAPI_LOGGER_TRACE(
              "this->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(channelId).at("
              "symbolId) = " +
              toString(this->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
          ...
          this->updateElementListWithUpdateMarketDepth(field, optionMap, snapshotBid, snapshotBidPrevious, snapshotAsk,
                                                       snapshotAskPrevious, elementList, false);
        }
        ...
        if (!elementList.empty()) {
          ...
          TimePoint time =
              shouldConflate ? this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                                       .at(channelId)
                                       .at(symbolId) +
  ...
  void processTrade(const WsConnection& wsConnection, const std::string& channelId, const std::string& symbolId,
                    Event& event, bool& shouldEmitEvent, const TimePoint& tp, const TimePoint& timeReceived,
                    const MarketDataMessage::TypeForData& input, const std::string& field,
    ...
    if (!shouldConflate || intervalChanged) {
      ...
      if (!elementList.empty()) {
        ...
        message.setType(Message::Type::MARKET_DATA_EVENTS);
        message.setRecapType(Message::RecapType::NONE);
        TimePoint time = shouldConflate ? this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                                              .at(channelId)
                                              .at(symbolId)
                                        : conflateTp;
        message.setTime(time);
  ...
  void updateOhlc(const WsConnection& wsConnection, const std::string& channelId, const std::string& symbolId,
  ...
  void setPingPongTimer(PingPongMethod method, WsConnection& wsConnection, wspp::connection_hdl hdl,
    ...
    if (wsConnection.status == WsConnection::Status::OPEN) {
      if (this->pingTimerByMethodByConnectionIdMap.find(wsConnection.id) != this->pingTimerByMethodByConnectionIdMap.end() &&
          this->pingTimerByMethodByConnectionIdMap.at(wsConnection.id).find(method) !=
              this->pingTimerByMethodByConnectionIdMap.at(wsConnection.id).end()) {
      ...
      this->pingTimerByMethodByConnectionIdMap[wsConnection.id][method] = this->serviceContextPtr->tlsClientPtr->set_timer(
          ...
          [wsConnection, that = shared_from_base<MarketDataService>(), hdl, pingMethod, pongTimeoutMilliSeconds,
           method](ErrorCode const& ec) {
            if (that->wsConnectionMap.find(wsConnection.id) != that->wsConnectionMap.end()) {
              if (ec) {
                ...
                if (that->wsConnectionMap.at(wsConnection.id).status == WsConnection::Status::OPEN) {
                  ...
                  if (that->pongTimeOutTimerByMethodByConnectionIdMap.find(wsConnection.id) !=
                          that->pongTimeOutTimerByMethodByConnectionIdMap.end() &&
                  ...
                  that->pongTimeOutTimerByMethodByConnectionIdMap[wsConnection.id][method] =
                      that->serviceContextPtr->tlsClientPtr->set_timer(
                          pongTimeoutMilliSeconds,
                          [wsConnection, that, hdl, pingMethod, pongTimeoutMilliSeconds, method](ErrorCode const& ec) {
                            if (that->wsConnectionMap.find(wsConnection.id) != that->wsConnectionMap.end()) {
                              if (ec) {
                                CCAPI_LOGGER_ERROR("wsConnection = " + toString(wsConnection) +
                                                   ", pong time out timer error: " + ec.message());
                                that->onError(Event::Type::SUBSCRIPTION_STATUS, Message::Type::GENERIC_ERROR, ec, "timer");
                              } else {
                                if (that->wsConnectionMap.at(wsConnection.id).status == WsConnection::Status::OPEN) {
                                  auto now = UtilTime::now();
                                  if (that->lastPongTpByMethodByConnectionIdMap.find(wsConnection.id) !=
                                          that->lastPongTpByMethodByConnectionIdMap.end() &&
                                    ...
                                    ErrorCode ec;
                                    that->close(thisWsConnection, hdl, websocketpp::close::status::normal, "pong timeout",
                                                ec);
                                    if (ec) {
                                      that->onError(Event::Type::SUBSCRIPTION_STATUS, Message::Type::GENERIC_ERROR, ec,
                                                    "shutdown");
                                    }
                                    that->shouldProcessRemainingMessageOnClosingByConnectionIdMap[thisWsConnection.id] =
                                        true;
                                  } else {
                                    auto thisWsConnection = wsConnection;
  ...
  virtual void onIncorrectStatesFound(WsConnection& wsConnection, wspp::connection_hdl hdl, const std::string& textMessage,
    ...
    std::string errorMessage = "incorrect states found: connection = " + toString(wsConnection) +
                               ...
                               ", exchangeSubscriptionId = " + exchangeSubscriptionId + ", reason = " + reason;
  ...
  void setConflateTimer(const TimePoint& previousConflateTp, const std::chrono::milliseconds& interval,
                        ...
                        const std::string& channelId, const std::string& symbolId, const std::string& field,
                        const std::map<std::string, std::string>& optionMap,
    ...
    if (wsConnection.status == WsConnection::Status::OPEN) {
      ...
      long waitMilliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
                                  previousConflateTp + interval + gracePeriod - std::chrono::system_clock::now())
                                  .count();
      if (waitMilliseconds > 0) {
        this->conflateTimerMapByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId] =
            this->serviceContextPtr->tlsClientPtr->set_timer(waitMilliseconds, [wsConnection, channelId, symbolId, field,
                                                                                optionMap, correlationIdList,
                                                                                previousConflateTp, interval, gracePeriod,
              ...
              if (this->wsConnectionMap.find(wsConnection.id) != this->wsConnectionMap.end()) {
                if (ec) {
                  ...
                  if (this->wsConnectionMap.at(wsConnection.id).status == WsConnection::Status::OPEN) {
                    ...
                    if (conflateTp > this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                                         .at(channelId)
                                         .at(symbolId)) {
                      Event event;
                      event.setType(Event::Type::SUBSCRIPTION_DATA);
                      ...
                      CCAPI_LOGGER_TRACE("elementList = " + toString(elementList));
                      this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id)
                          .at(channelId)
                          .at(symbolId) = conflateTp;
                      std::vector<Message> messageList;
                      if (!elementList.empty()) {
                        Message message;
                    ...
                    this->setConflateTimer(conflateTp, interval, gracePeriod, wsConnection, channelId, symbolId, field,
                                           optionMap, correlationIdList);
                  }
                }
              }
  ...
  void processSuccessfulTextMessage(const Request& request, const std::string& textMessage,
    ...
    CCAPI_LOGGER_FUNCTION_ENTER;
    const std::vector<MarketDataMessage>& marketDataMessageList =
        this->convertTextMessageToMarketDataMessage(request, textMessage, timeReceived);
  ...
  void processTrade(Event& event, const TimePoint& tp, const TimePoint& timeReceived,
                    const MarketDataMessage::TypeForData& input, const std::vector<std::string>& correlationIdList) {
    std::vector<Message> messageList;
    std::vector<Element> elementList;
  ...
  virtual std::vector<MarketDataMessage> convertTextMessageToMarketDataMessage(const Request& request,
  ...
  std::map<std::string, std::map<std::string, std::map<std::string, std::map<std::string, std::string>>>>
      optionMapByConnectionIdChannelIdSymbolIdMap;
  ...
  std::map<std::string, std::map<std::string, std::map<std::string, std::vector<Subscription>>>>
      subscriptionListByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, std::vector<std::string>>>>
      correlationIdListByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, std::string>>>
      channelIdSymbolIdByConnectionIdExchangeSubscriptionIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, std::map<Decimal, std::string>>>>
      snapshotBidByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, std::map<Decimal, std::string>>>>
      snapshotAskByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, std::map<Decimal, std::string>>>>
  ...
  std::map<std::string, std::map<std::string, std::map<std::string, bool>>>
      processedInitialSnapshotByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, bool>>>
      processedInitialTradeByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, bool>>>
      l2UpdateIsReplaceByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, bool> shouldProcessRemainingMessageOnClosingByConnectionIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, TimePoint>>>
      previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap;
  std::map<std::string, std::map<std::string, std::map<std::string, TimerPtr>>>
      conflateTimerMapByConnectionIdChannelIdSymbolIdMap;
