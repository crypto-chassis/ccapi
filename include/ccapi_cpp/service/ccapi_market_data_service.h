...
namespace ccapi {
                ...
                for (const auto &x : this->groupSubscriptionListByInstrumentGroup(subscriptionList)) {
                    ...
                    wspp::lib::asio::post(
                            ...
                            [that = shared_from_base<MarketDataService>(), instrumentGroup, subscriptionListGivenInstrumentGroup]() {
                                std::map<std::string, std::vector<std::string> > wsConnectionIdListByInstrumentGroupMap = invertMapMulti(
                                        that->instrumentGroupByWsConnectionIdMap);
                                if (wsConnectionIdListByInstrumentGroupMap.find(instrumentGroup) !=
                                    ...
                                    auto wsConnectionId = wsConnectionIdListByInstrumentGroupMap.at(instrumentGroup).at(
                                            0);
                                    ...
                                    for (const auto &subscription : subscriptionListGivenInstrumentGroup) {
                                        ...
                                        if (that->subscriptionStatusByInstrumentGroupInstrumentMap[instrumentGroup].find(
                                                instrument) !=
                                            that->subscriptionStatusByInstrumentGroupInstrumentMap[instrumentGroup].end()) {
                                            that->onError(Event::Type::SUBSCRIPTION_STATUS,
                                                          Message::Type::SUBSCRIPTION_FAILURE,
                                    ...
                                    WsConnection wsConnection(url, instrumentGroup,
    ...
    protected:
        ...
        groupSubscriptionListByInstrumentGroup(const std::vector<Subscription> &subscriptionList) {
        ...
        virtual void onOpen(wspp::connection_hdl hdl) {
            ...
            if (this->enableCheckPingPongWebsocketProtocolLevel) {
                this->setPingPongTimer(PingPongMethod::WEBSOCKET_PROTOCOL_LEVEL, wsConnection, hdl,
                                       [that = shared_from_base<MarketDataService>()](wspp::connection_hdl hdl,
                                                                                      ErrorCode &ec) {
                                           that->ping(hdl, "", ec);
                                       });
            }
        ...
        void prepareSubscription(const WsConnection &wsConnection, const Subscription &subscription) {
            ...
            if (field == CCAPI_MARKET_DEPTH) {
                if (this->name == CCAPI_EXCHANGE_NAME_KRAKEN || this->name == CCAPI_EXCHANGE_NAME_BITFINEX
                    ...
                    this->name == CCAPI_EXCHANGE_NAME_BINANCE_FUTURES || this->name == CCAPI_EXCHANGE_NAME_HUOBI ||
                    this->name == CCAPI_EXCHANGE_NAME_HUOBI_USDT_PERPETUAL || this->name == CCAPI_EXCHANGE_NAME_OKEX ||
                    ...
                    if (marketDepthRequested <= 20) {
                        channelId = std::string(CCAPI_WEBSOCKET_ERISX_CHANNEL_TOP_OF_BOOK_MARKET_DATA_SUBSCRIBE) + "?" +
                                    CCAPI_MARKET_DEPTH_SUBSCRIBED_TO_EXCHANGE + "=" +
        ...
        void onFail_(WsConnection &wsConnection) {
            ...
            this->connectRetryOnFailTimerByConnectionIdMap[thisWsConnection.id] =
                    this->serviceContextPtr->tlsClientPtr->set_timer(
                            seconds * 1000,
                            [thisWsConnection, that = shared_from_base<MarketDataService>(), urlBase](
                                    ErrorCode const &ec) {
                                if (that->wsConnectionMap.find(thisWsConnection.id) == that->wsConnectionMap.end()) {
                                    if (ec) {
                                        ...
                                        that->onError(Event::Type::SUBSCRIPTION_STATUS, Message::Type::GENERIC_ERROR,
                                                      ec, "timer");
                                    } else {
        ...
        void clearStates(WsConnection &wsConnection) {
            ...
            if (this->pingTimerByMethodByConnectionIdMap.find(wsConnection.id) !=
                this->pingTimerByMethodByConnectionIdMap.end()) {
...
#if defined(CCAPI_ENABLE_EXCHANGE_HUOBI) || defined(CCAPI_ENABLE_EXCHANGE_HUOBI_USDT_PERPETUAL) || defined(CCAPI_ENABLE_EXCHANGE_OKEX) || defined(CCAPI_ENABLE_EXCHANGE_OKEX_PERPETUAL)
                if (this->name == CCAPI_EXCHANGE_NAME_HUOBI || this->name == CCAPI_EXCHANGE_NAME_HUOBI_USDT_PERPETUAL ||
                    this->name == CCAPI_EXCHANGE_NAME_OKEX_PERPETUAL || this->name == CCAPI_EXCHANGE_NAME_OKEX) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < payload.size(); ++i) {
                            ss << std::setw(2)
                               << static_cast<unsigned>(reinterpret_cast<const uint8_t *>(&payload[0])[i]);
                        }
        ...
        virtual void
        onTextMessage(wspp::connection_hdl hdl, const std::string &textMessage, const TimePoint &timeReceived) {
            CCAPI_LOGGER_FUNCTION_ENTER;
            WsConnection &wsConnection = this->getWsConnectionFromConnectionPtr(
                    this->serviceContextPtr->tlsClientPtr->get_con_from_hdl(hdl));
                    ...
                    if (wsMessage.type == MarketDataMessage::Type::MARKET_DATA_EVENTS) {
                        if (wsMessage.recapType == MarketDataMessage::RecapType::NONE
                            ...
                            CCAPI_LOGGER_WARN("late websocket message: timeReceived = " + toString(timeReceived) +
                                              ", wsMessage.tp = " + toString(wsMessage.tp) + ", wsConnection = " +
                                              toString(wsConnection));
                        }
                        event.setType(Event::Type::SUBSCRIPTION_DATA);
                        std::string exchangeSubscriptionId = wsMessage.exchangeSubscriptionId;
                        std::string channelId =
                                this->channelIdSymbolIdByConnectionIdExchangeSubscriptionIdMap.at(wsConnection.id).at(
                                        exchangeSubscriptionId).at(CCAPI_CHANNEL_ID);
                        std::string symbolId =
                                this->channelIdSymbolIdByConnectionIdExchangeSubscriptionIdMap.at(wsConnection.id).at(
                                        exchangeSubscriptionId).at(CCAPI_SYMBOL_ID);
                        auto field = this->fieldByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(channelId).at(
                                symbolId);
                        ...
                        auto optionMap = this->optionMapByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(
                                channelId).at(symbolId);
                        CCAPI_LOGGER_TRACE("optionMap = " + toString(optionMap));
                        auto correlationIdList =
                                this->correlationIdListByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(
                                        channelId).at(symbolId);
                        ...
                        if (wsMessage.data.find(MarketDataMessage::DataType::BID) != wsMessage.data.end()
                            ...
                            if (this->processedInitialSnapshotByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId]
                                ...
                                this->processUpdateSnapshot(wsConnection, channelId, symbolId, event, shouldEmitEvent,
                                                            wsMessage.tp,
                                                            timeReceived, wsMessage.data, field, optionMap,
                                                            correlationIdList,
                                ...
                                if (this->sessionOptions.enableCheckOrderBookChecksum) {
                                    ...
                                    if (!this->checkOrderBookChecksum(snapshotBid, snapshotAsk,
                                                                      receivedOrderBookChecksumStr,
                                        ...
                                        this->onIncorrectStatesFound(wsConnection, hdl, textMessage, timeReceived,
                                ...
                                if (this->sessionOptions.enableCheckOrderBookCrossed) {
                                    bool shouldProcessRemainingMessage = true;
                                    if (!this->checkOrderBookCrossed(snapshotBid, snapshotAsk,
                                                                     shouldProcessRemainingMessage)) {
                                        CCAPI_LOGGER_ERROR(
                                                "lastNToString(snapshotBid, 1) = " + lastNToString(snapshotBid, 1));
                                        CCAPI_LOGGER_ERROR(
                                                "firstNToString(snapshotAsk, 1) = " + firstNToString(snapshotAsk, 1));
                                        this->onIncorrectStatesFound(wsConnection, hdl, textMessage, timeReceived,
                                                                     exchangeSubscriptionId,
                                ...
                                this->processInitialSnapshot(wsConnection, channelId, symbolId, event, shouldEmitEvent,
                                                             wsMessage.tp,
                                                             timeReceived, wsMessage.data, field, optionMap,
                                                             correlationIdList,
                                                             snapshotBid, snapshotAsk);
        ...
        void updateElementListWithUpdateMarketDepth(const std::string &field,
            ...
            if (field == CCAPI_MARKET_DEPTH) {
                ...
                if (optionMap.at(CCAPI_MARKET_DEPTH_RETURN_UPDATE) == CCAPI_MARKET_DEPTH_RETURN_UPDATE_ENABLE) {
                    CCAPI_LOGGER_TRACE(
                            "lastNSame = " + toString(lastNSame(snapshotBid, snapshotBidPrevious, maxMarketDepth)));
                    CCAPI_LOGGER_TRACE(
                            "firstNSame = " + toString(firstNSame(snapshotAsk, snapshotAskPrevious, maxMarketDepth)));
                    const std::map<Decimal, std::string> &snapshotBidUpdate = this->calculateMarketDepthUpdate(true,
                                                                                                               snapshotBid,
                                                                                                               snapshotBidPrevious,
                                                                                                               maxMarketDepth);
                    ...
                    const std::map<Decimal, std::string> &snapshotAskUpdate = this->calculateMarketDepthUpdate(false,
                                                                                                               snapshotAsk,
                                                                                                               snapshotAskPrevious,
                                                                                                               maxMarketDepth);
                ...
                for (const auto &x : input) {
                    ...
                    if (type == MarketDataMessage::DataType::TRADE) {
                        for (const auto &y : detail) {
                            ...
                            element.insert(CCAPI_IS_BUYER_MAKER,
                                           y.at(MarketDataMessage::DataFieldType::IS_BUYER_MAKER));
                            elementList.push_back(std::move(element));
                        }
                    } else {
                        CCAPI_LOGGER_WARN(
        ...
        void prepareConnect(WsConnection &wsConnection) {
            if (this->name == CCAPI_EXCHANGE_NAME_KUCOIN) {
                ...
                this->sendRequest(host, port, req,
                                  [wsConnection, that = shared_from_base<MarketDataService>()](
                            ...
                            if (statusCode / 100 == 2) {
                                ...
                                try {
                                    ...
                                    that->extraPropertyByConnectionIdMap[thisWsConnection.id].insert({
                                                                                                             {"pingInterval", std::to_string(
                                                                                                                     instanceServer["pingInterval"].GetInt())},
                                                                                                             {"pingTimeout",  std::to_string(
                                                                                                                     instanceServer["pingTimeout"].GetInt())}
                                                                                                     });
        ...
        void connect(WsConnection &wsConnection) {
            ...
            this->serviceContextPtr->tlsClientPtr->set_tls_init_handler(
                    std::bind(&MarketDataService::onTlsInit, shared_from_base<MarketDataService>(),
                              std::placeholders::_1));
            ...
            CCAPI_LOGGER_DEBUG(
                    "this->instrumentGroupByWsConnectionIdMap = " + toString(this->instrumentGroupByWsConnectionIdMap));
            con->set_open_handler(std::bind(&MarketDataService::onOpen, shared_from_base<MarketDataService>(),
                                            std::placeholders::_1));
            con->set_fail_handler(std::bind(&MarketDataService::onFail, shared_from_base<MarketDataService>(),
                                            std::placeholders::_1));
            con->set_close_handler(std::bind(&MarketDataService::onClose, shared_from_base<MarketDataService>(),
                                             std::placeholders::_1));
            con->set_message_handler(
                    std::bind(&MarketDataService::onMessage, shared_from_base<MarketDataService>(),
            ...
            con->set_ping_handler(
                    std::bind(&MarketDataService::onPing, shared_from_base<MarketDataService>(), std::placeholders::_1,
                              std::placeholders::_2));
        ...
        void
        copySnapshot(bool isBid, const std::map<Decimal, std::string> &original, std::map<Decimal, std::string> &copy,
        ...
        void processInitialSnapshot(const WsConnection &wsConnection, const std::string &channelId,
                                    const std::string &symbolId, Event &event, bool &shouldEmitEvent,
                                    const TimePoint &tp,
            ...
            if (shouldConflate) {
                ...
                CCAPI_LOGGER_TRACE(
                        ...
                        toString(this->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(
                                wsConnection.id).at(channelId).at(symbolId)));
                CCAPI_LOGGER_TRACE(
                        "this->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(channelId).at(symbolId) = " +
                        toString(this->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(
                                wsConnection.id).at(channelId).at(symbolId)));
                ...
                if (optionMap.at(
                    ...
                    this->setConflateTimer(previousConflateTp, interval, gracePeriod, wsConnection, channelId, symbolId,
                                           field,
        ...
        void processUpdateSnapshot(const WsConnection &wsConnection, const std::string &channelId,
            ...
            if (this->processedInitialSnapshotByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId]) {
                ...
                CCAPI_LOGGER_TRACE(
                        "lastNToString(snapshotBid, " + toString(maxMarketDepth) + ") = " +
                        lastNToString(snapshotBid, maxMarketDepth));
                CCAPI_LOGGER_TRACE(
                        "firstNToString(snapshotAsk, " + toString(maxMarketDepth) + ") = " +
                        firstNToString(snapshotAsk, maxMarketDepth));
                ...
                bool intervalChanged = shouldConflate
                                       && conflateTp
                                          > this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(
                        wsConnection.id).at(channelId).at(
                        symbolId);
                ...
                if (!shouldConflate || intervalChanged) {
                    ...
                    if (shouldConflate && intervalChanged) {
                        const std::map<Decimal, std::string> &snapshotBidPreviousPrevious = this
                                ->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(
                                channelId).at(
                                symbolId);
                        const std::map<Decimal, std::string> &snapshotAskPreviousPrevious = this
                                ->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(
                                channelId).at(
                                symbolId);
                        ...
                        this->previousConflateSnapshotBidByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(
                                channelId).at(
                                symbolId) = snapshotBidPrevious;
                        this->previousConflateSnapshotAskByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(
                                channelId).at(
                        ...
                        this->updateElementListWithUpdateMarketDepth(field, optionMap, snapshotBid, snapshotBidPrevious,
                                                                     snapshotAsk, snapshotAskPrevious, elementList,
                                                                     false);
                    }
                    ...
                    if (!elementList.empty()) {
                        ...
                        TimePoint time = shouldConflate ?
                                         this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(
                                                 wsConnection.id).at(channelId).at(symbolId) +
                                         std::chrono::milliseconds(std::stoll(optionMap.at(
                    ...
                    if (shouldConflate) {
                        this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(wsConnection.id).at(
                                channelId).at(
                                symbolId) = conflateTp;
                    }
                }
        ...
        void processTrade(const WsConnection &wsConnection, const std::string &channelId,
            ...
            if (!this->processedInitialTradeByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId]) {
                if (shouldConflate) {
                    ...
                    if (optionMap.at(
                        ...
                        auto gracePeriod = std::chrono::milliseconds(std::stoi(optionMap.at(
                                CCAPI_CONFLATE_GRACE_PERIOD_MILLISECONDS)));
                        this->setConflateTimer(previousConflateTp, interval, gracePeriod, wsConnection, channelId,
                                               symbolId, field,
                                               optionMap, correlationIdList);
                    }
                }
            ...
            if (!shouldConflate || intervalChanged) {
                ...
                if (!elementList.empty()) {
                    ...
                    TimePoint time = shouldConflate
                                     ? this->previousConflateTimeMapByConnectionIdChannelIdSymbolIdMap.at(
                                    wsConnection.id).at(channelId).at(symbolId) : conflateTp;
                    message.setTime(time);
                    message.setElementList(elementList);
                    message.setCorrelationIdList(correlationIdList);
                    messageList.push_back(std::move(message));
                    ...
                    if (type == MarketDataMessage::DataType::TRADE) {
                        for (const auto &y : detail) {
                            ...
                            if (this->openByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId].empty()) {
                                ...
                                if (decimalPrice >
                                    this->highByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId]) {
                                    this->highByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId] = decimalPrice;
                                }
                                if (decimalPrice <
                                    this->lowByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId]) {
        ...
        virtual void
        alignSnapshot(std::map<Decimal, std::string> &snapshotBid, std::map<Decimal, std::string> &snapshotAsk,
        ...
        void setPingPongTimer(PingPongMethod method,
            ...
            if (wsConnection.status == WsConnection::Status::OPEN) {
                ...
                this->pingTimerByMethodByConnectionIdMap[wsConnection.id][method] =
                        this->serviceContextPtr->tlsClientPtr->set_timer(
                                pingIntervalMilliSeconds - pongTimeoutMilliSeconds,
                                [wsConnection, that = shared_from_base<MarketDataService>(), hdl, pingMethod, pongTimeoutMilliSeconds, method](
                                        ErrorCode const &ec) {
                                    if (that->wsConnectionMap.find(wsConnection.id) != that->wsConnectionMap.end()) {
                                        if (ec) {
                                            CCAPI_LOGGER_ERROR("wsConnection = " + toString(wsConnection) +
                                                               ", ping timer error: " + ec.message());
                                            that->onError(Event::Type::SUBSCRIPTION_STATUS,
                                                          Message::Type::GENERIC_ERROR, ec, "timer");
                                        } else {
                                            if (that->wsConnectionMap.at(wsConnection.id).status ==
                                                WsConnection::Status::OPEN) {
                                                ErrorCode ec;
                                                pingMethod(hdl, ec);
                                                if (ec) {
                                                    that->onError(Event::Type::SUBSCRIPTION_STATUS,
                                                                  Message::Type::GENERIC_ERROR, ec, "ping");
                                                }
                                                if (pongTimeoutMilliSeconds <= 0) {
                                                    return;
                                                }
                                                if (that->pongTimeOutTimerByMethodByConnectionIdMap.find(
                                                        wsConnection.id) !=
                                                    that->pongTimeOutTimerByMethodByConnectionIdMap.end() &&
                                                    that->pongTimeOutTimerByMethodByConnectionIdMap.at(
                                                            wsConnection.id).find(method) !=
                                                    that->pongTimeOutTimerByMethodByConnectionIdMap.at(
                                                            wsConnection.id).end()) {
                                                    that->pongTimeOutTimerByMethodByConnectionIdMap.at(
                                                            wsConnection.id).at(method)->cancel();
                                                }
                                                that->pongTimeOutTimerByMethodByConnectionIdMap[wsConnection.id][method] = that->serviceContextPtr->tlsClientPtr->set_timer(
                                                        ...
                                                        [wsConnection, that, hdl, pingMethod, pongTimeoutMilliSeconds, method](
                                                                ErrorCode const &ec) {
                                                            if (that->wsConnectionMap.find(wsConnection.id) !=
                                                                that->wsConnectionMap.end()) {
                                                                if (ec) {
                                                                    CCAPI_LOGGER_ERROR(
                                                                            "wsConnection = " + toString(wsConnection) +
                                                                            ", pong time out timer error: " +
                                                                            ec.message());
                                                                    that->onError(Event::Type::SUBSCRIPTION_STATUS,
                                                                                  Message::Type::GENERIC_ERROR, ec,
                                                                                  "timer");
                                                                } else {
                                                                    if (that->wsConnectionMap.at(
                                                                            wsConnection.id).status ==
                                                                        WsConnection::Status::OPEN) {
                                                                        auto now = UtilTime::now();
                                                                        if (that->lastPongTpByMethodByConnectionIdMap.find(
                                                                                wsConnection.id) !=
                                                                            that->lastPongTpByMethodByConnectionIdMap.end() &&
                                                                            that->lastPongTpByMethodByConnectionIdMap.at(
                                                                                    wsConnection.id).find(method) !=
                                                                            that->lastPongTpByMethodByConnectionIdMap.at(
                                                                                    wsConnection.id).end() &&
            ...
            if (wsConnection.status == WsConnection::Status::OPEN) {
                ...
                if (waitMilliseconds > 0) {
                    this->conflateTimerMapByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId] =
                            this->serviceContextPtr->tlsClientPtr->set_timer(
                                    waitMilliseconds,
                                    [wsConnection, channelId, symbolId, field, optionMap, correlationIdList, previousConflateTp, interval, gracePeriod, this](
                                            ErrorCode const &ec) {
                                        if (this->wsConnectionMap.find(wsConnection.id) !=
                                            this->wsConnectionMap.end()) {
                                            if (ec) {
                                                CCAPI_LOGGER_ERROR("wsConnection = " + toString(wsConnection) +
                                                                   ", conflate timer error: " + ec.message());
                                                this->onError(Event::Type::SUBSCRIPTION_STATUS,
                                                              Message::Type::GENERIC_ERROR, ec, "timer");
                                            } else {
                                                if (this->wsConnectionMap.at(wsConnection.id).status ==
                                                    WsConnection::Status::OPEN) {
                                                    ...
                                                    if (conflateTp >
                                                        ...
                                                        if (field == CCAPI_MARKET_DEPTH) {
                                                            ...
                                                            this->updateElementListWithUpdateMarketDepth(field,
                                                                                                         optionMap,
                                                                                                         snapshotBid,
                                                                                                         std::map<Decimal, std::string>(),
                                                                                                         snapshotAsk,
                                                                                                         std::map<Decimal, std::string>(),
                                                                                                         elementList,
                                                                                                         true);
                                                        } else if (field == CCAPI_TRADE) {
                                                            this->updateElementListWithOhlc(wsConnection, channelId,
                                                                                            symbolId, field,
                                                                                            elementList);
                                                        }
                                                        CCAPI_LOGGER_TRACE("elementList = " + toString(elementList));
                                                        ...
                                                        if (!elementList.empty()) {
                                                            ...
                                                            message.setType(Message::Type::MARKET_DATA_EVENTS);
                                                            message.setRecapType(Message::RecapType::NONE);
                                                            message.setTime(field == CCAPI_MARKET_DEPTH ? conflateTp
                                                                                                        : previousConflateTp);
                                                            message.setElementList(elementList);
                                                            message.setCorrelationIdList(correlationIdList);
                                                            messageList.push_back(std::move(message));
                                                    ...
                                                    this->setConflateTimer(conflateTp, interval, gracePeriod,
                                                                           wsConnection, channelId, symbolId, field,
                                                                           optionMap, correlationIdList);
                                                }
                                            }
                                        }
        ...
        virtual std::vector<MarketDataMessage>
        processTextMessage(wspp::connection_hdl hdl, const std::string &textMessage,
