  ...
  virtual ~MarketDataService() {
   ...
   protected:
          ...
          if (marketDataMessage.type == MarketDataMessage::Type::MARKET_DATA_EVENTS) {
            ...
            if (marketDataMessage.data.find(MarketDataMessage::DataType::BID) != marketDataMessage.data.end() ||
              ...
              if (this->processedInitialSnapshotByConnectionIdChannelIdSymbolIdMap[wsConnection.id][channelId][symbolId] &&
                ...
                this->processUpdateSnapshot(wsConnection, channelId, symbolId, event, shouldEmitEvent, marketDataMessage.tp, timeReceived,
    ...
    void processInitialSnapshot(
        const WsConnection& wsConnection, const std::string& channelId, const std::string& symbolId, Event& event, bool& shouldEmitEvent, const TimePoint& tp,
        const TimePoint& timeReceived, MarketDataMessage::TypeForData& input, const std::string& field, const std::map<std::string, std::string>& optionMap,
        const std::vector<std::string>& correlationIdList, std::map<Decimal, std::string>& snapshotBid, std::map<Decimal, std::string>& snapshotAsk) {
      snapshotBid.clear();
      snapshotAsk.clear();
      int maxMarketDepth = std::stoi(optionMap.at(CCAPI_MARKET_DEPTH_MAX));
      for (auto& x : input) {
      ...
      if (shouldConflate) {
        ...
        TimePoint previousConflateTp = UtilTime::makeTimePointFromMilliseconds(
            ...
            std::stoi(optionMap.at(CCAPI_CONFLATE_INTERVAL_MILLISECONDS)) * std::stoi(optionMap.at(CCAPI_CONFLATE_INTERVAL_MILLISECONDS)));
