...
class MarketDataServiceFTX CCAPI_FINAL : public MarketDataService {
 public:
  MarketDataServiceFTX(std::function<void(Event& event)> wsEventHandler, SessionOptions sessionOptions, SessionConfigs sessionConfigs,
    ...
    this->getRecentTradesTarget = "/products/<product-id>/trades";  // todo update
  }
  ...
  std::vector<std::string> createRequestStringList(const WsConnection& wsConnection) override {
    ...
    std::vector<MarketDataMessage> processTextMessage(WsConnection & wsConnection, wspp::connection_hdl hdl, const std::string& textMessage,
      ...
      if (type == "update") {
        ...
        auto exchangeSubscriptionId = std::string(CCAPI_WEBSOCKET_FTX_CHANNEL_ORDER_BOOK) + "|" + symbolId;
        ...
        dataPoint.insert({MarketDataMessage::DataFieldType::IS_BUYER_MAKER, std::string(document["data"]["side"].GetString()) == "buy" ? "1" : "0"});
    ...
    std::vector<MarketDataMessage> convertTextMessageToMarketDataMessage(const Request& request, const std::string& textMessage, const TimePoint& timeReceived)
        override {
      rj::Document document;
      document.Parse(textMessage.c_str());
      std::vector<MarketDataMessage> marketDataMessageList;
