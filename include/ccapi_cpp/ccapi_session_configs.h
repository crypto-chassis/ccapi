...
public:
  ...
  explicit SessionConfigs(
      std::map<std::string, std::map<std::string, std::string>>
          exchangeInstrumentSymbolMap,
      std::map<std::string, std::map<std::string, std::string>>
          exchangeInstrumentSymbolMapRest = {},
      std::map<std::string, std::string> credential = {})
      : exchangeInstrumentSymbolMap(exchangeInstrumentSymbolMap),
        exchangeInstrumentSymbolMapRest(exchangeInstrumentSymbolMapRest),
        credential(credential) {
    this->exchangeSymbolInstrumentMap =
        this->invertInstrumentSymbolMap(exchangeInstrumentSymbolMap);
    this->exchangeSymbolInstrumentMapRest =
        this->invertInstrumentSymbolMap(exchangeInstrumentSymbolMapRest);
    this->updateExchangeInstrumentMap();
    this->updateExchangeInstrumentMapRest();
  }
  const std::map<std::string, std::map<std::string, std::string>> &
  getExchangeInstrumentSymbolMap() const {
    return exchangeInstrumentSymbolMap;
  }
  const std::map<std::string, std::map<std::string, std::string>> &
  getExchangeInstrumentSymbolMapRest() const {
  ...
  getExchangeInstrumentMap() const {
    return exchangeInstrumentMap;
  }
  const std::map<std::string, std::vector<std::string>> &
  getExchangeInstrumentMapRest() const {
    return exchangeInstrumentMapRest;
  }
  const std::map<std::string, std::vector<std::string>> &
  getExchangeFieldMap() const {
    return exchangeFieldMap;
  }
  const std::map<std::string, std::map<std::string, std::string>> &
  getExchangeFieldWebsocketChannelMap() const {
    return exchangeFieldWebsocketChannelMap;
  }
  const std::map<std::string, std::vector<int>> &
  getWebsocketAvailableMarketDepth() const {
  ...
  const std::map<std::string, std::map<std::string, std::string>> &
  getExchangeSymbolInstrumentMap() const {
    return exchangeSymbolInstrumentMap;
  }
  const std::map<std::string, std::map<std::string, std::string>> &
  getExchangeSymbolInstrumentMapRest() const {
...
private:
  void updateExchangeInstrumentMap() {
    ...
    std::map<std::string, std::string> fieldWebsocketChannelMapGemini = {
        ...
        {CCAPI_MARKET_DEPTH,
         std::string(CCAPI_WEBSOCKET_GEMINI_PARAMETER_BIDS) + "," +
    ...
    std::map<std::string, std::string> fieldWebsocketChannelMapFTX = {
        ...
        {CCAPI_MARKET_DEPTH, CCAPI_WEBSOCKET_FTX_CHANNEL_ORDER_BOOK},
    };
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapCoinbase) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_COINBASE].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketParameter : fieldWebsocketChannelMapGemini) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_GEMINI].push_back(
          fieldWebsocketParameter.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapKraken) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_KRAKEN].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapBitstamp) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_BITSTAMP].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapBitfinex) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_BITFINEX].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapBitmex) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_BITMEX].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel :
         fieldWebsocketChannelMapBinanceUs) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_BINANCE_US].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapBinance) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_BINANCE].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel :
         fieldWebsocketChannelMapBinanceFutures) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_BINANCE_FUTURES].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapHuobi) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_HUOBI].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel :
         fieldWebsocketChannelMapHuobiUsdtSwap) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_HUOBI_USDT_SWAP].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapOkex) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_OKEX].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapErisx) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_ERISX].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapKucoin) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_KUCOIN].push_back(
          fieldWebsocketChannel.first);
    }
    for (auto const &fieldWebsocketChannel : fieldWebsocketChannelMapFTX) {
      this->exchangeFieldMap[CCAPI_EXCHANGE_NAME_FTX].push_back(
          fieldWebsocketChannel.first);
    }
    CCAPI_LOGGER_TRACE("this->exchangeFieldMap = " +
                       toString(this->exchangeFieldMap));
    this->exchangeFieldWebsocketChannelMap = {
        ...
        {CCAPI_EXCHANGE_NAME_BINANCE_FUTURES,
         fieldWebsocketChannelMapBinanceFutures},
        {CCAPI_EXCHANGE_NAME_HUOBI, fieldWebsocketChannelMapHuobi},
        {CCAPI_EXCHANGE_NAME_HUOBI_USDT_SWAP,
         fieldWebsocketChannelMapHuobiUsdtSwap},
    ...
    this->websocketAvailableMarketDepth = {
        {CCAPI_EXCHANGE_NAME_KRAKEN,
         std::vector<int>({10, 25, 100, 500, 1000})},
    ...
    this->urlWebsocketBase = {
        ...
        //  Kucoin has dynamic websocket url. Here it is only a placeholder for
        //  subscription grouping purposes.
        {CCAPI_EXCHANGE_NAME_KUCOIN,
         "CCAPI_EXCHANGE_NAME_KUCOIN_URL_WEBSOCKET_BASE"}};
    this->initialSequenceByExchangeMap = {{CCAPI_EXCHANGE_NAME_GEMINI, 0},
                                          {CCAPI_EXCHANGE_NAME_BITFINEX, 1}};
  }
  void updateExchangeInstrumentMapRest() {
    ...
    this->urlRestBase = {
        ...
        {CCAPI_EXCHANGE_NAME_BINANCE_FUTURES,
         CCAPI_BINANCE_FUTURES_URL_REST_BASE},
        {CCAPI_EXCHANGE_NAME_HUOBI, CCAPI_HUOBI_URL_REST_BASE},
        {CCAPI_EXCHANGE_NAME_HUOBI_USDT_SWAP,
         CCAPI_HUOBI_USDT_SWAP_URL_REST_BASE},
  ...
  std::map<std::string, std::map<std::string, std::string>>
  invertInstrumentSymbolMap(
      std::map<std::string, std::map<std::string, std::string>>
          exchangeInstrumentSymbolMap) {
    std::map<std::string, std::map<std::string, std::string>>
        exchangeSymbolInstrumentMap;
    for (const auto &x : exchangeInstrumentSymbolMap) {
      exchangeSymbolInstrumentMap.insert(
          std::make_pair(x.first, invertMap(x.second)));
    }
    return exchangeSymbolInstrumentMap;
  }
  std::map<std::string, std::map<std::string, std::string>>
  ...
  std::map<std::string, std::map<std::string, std::string>>
      exchangeSymbolInstrumentMap;
  std::map<std::string, std::map<std::string, std::string>>
      exchangeSymbolInstrumentMapRest;
  ...
  std::map<std::string, std::map<std::string, std::string>>
      exchangeFieldWebsocketChannelMap;
