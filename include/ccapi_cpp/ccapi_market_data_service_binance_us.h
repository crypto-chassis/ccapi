...
namespace ccapi {
class MarketDataServiceBinanceUs final : public MarketDataService {
 public:
  MarketDataServiceBinanceUs(SubscriptionList subscriptionList, std::function<void(Event& event)> wsEventHandler, SessionOptions sessionOptions, SessionConfigs sessionConfigs, ServiceContext& serviceContext): MarketDataService(subscriptionList, wsEventHandler, sessionOptions, sessionConfigs, serviceContext) {
...
//    this->onOpen_2(hdl);
    ...
    for (const auto & subscriptionListByChannelIdProductId : this->subscriptionListByConnectionIdChannelIdProductIdMap.at(wsConnection.id)) {
      ...
      for (auto & subscriptionListByInstrument : subscriptionListByChannelIdProductId.second) {
        ...
        if (channelId.rfind(CCAPI_EXCHANGE_NAME_WEBSOCKET_BINANCE_US_CHANNEL_PARTIAL_BOOK_DEPTH, 0) == 0) {
        ...
        std::string exchangeSubscriptionId = productId + "@" + std::string(CCAPI_EXCHANGE_NAME_WEBSOCKET_BINANCE_US_CHANNEL_PARTIAL_BOOK_DEPTH);
    ...
    subscription.AddMember("id", rj::Value(exchangeJsonPayloadIdByConnectionIdMap[wsConnection.id]).Move(), allocator);
