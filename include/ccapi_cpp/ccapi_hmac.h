...
#define SHA1AddLength(context, length)                     \
    (addTemp1 = (context)->Length_Low,                      \
     (context)->Corrupted =                                \
        (((context)->Length_Low += (length)) < addTemp1) && \
...
#define SHA224_256AddLength(context, length)               \
  (addTemp2 = (context)->Length_Low, (context)->Corrupted = \
    (((context)->Length_Low += (length)) < addTemp2) &&     \
    (++(context)->Length_High == 0) ? 1 : 0)
...
#define SHA384_512AddLength(context, length)                   \
   (addTemp4 = context->Length_Low, context->Corrupted =        \
    ((context->Length_Low += length) < addTemp4) &&             \
    (++context->Length_High == 0) ? 1 : 0)
...
//  https://github.com/Yubico/yubico-c-client/blob/ykclient-2.15/sha384-512.c
 public:
  ...
  static std::string hmac(const ShaVersion shaVersion, const std::string& key, const std::string& msg, bool returnHex = false) {
    ...
    int err = yubico::hmac(whichSha,
                     reinterpret_cast<unsigned char*>(const_cast<char*>(msg.c_str())),
                     msg.length(),
