#ifndef INCLUDE_CCAPI_CPP_CCAPI_MARKET_DATA_SERVICE_HUOBI_H_
#define INCLUDE_CCAPI_CPP_CCAPI_MARKET_DATA_SERVICE_HUOBI_H_
 ...
 private:
  ...
  void onOpen(wspp::connection_hdl hdl) override {
    ...
    for (const auto & subscriptionListByChannelIdProductId : this->subscriptionListByConnectionIdChannelIdProductIdMap.at(wsConnection.id)) {
      ...
      for (auto & subscriptionListByInstrument : subscriptionListByChannelIdProductId.second) {
        ...
        if (channelId == CCAPI_EXCHANGE_NAME_WEBSOCKET_HUOBI_CHANNEL_MARKET_DEPTH) {
        ...
        document.AddMember("sub", exchangeSubscriptionId, allocator);
  ...
  std::vector<MarketDataMessage> processTextMessage(wspp::connection_hdl hdl, std::string& textMessage, TimePoint& timeReceived) override {
    ...
    if (document.IsObject() && document.HasMember("status") && document.HasMember("subbed")) {
      ...
      if (std::regex_search(channelId.begin(), channelId.end(), match, std::regex(CCAPI_EXCHANGE_NAME_WEBSOCKET_HUOBI_CHANNEL_TRADE_DETAIL_REGEX))) {
        //id always increasing?
        wsMessage.recapType = MarketDataMessage::RecapType::NONE;
        wsMessage.tp = TimePoint(std::chrono::milliseconds(data["tick"]["ts"].GetInt64()));
      ...
      } else if (std::regex_search(channelId.begin(), channelId.end(), match, std::regex(CCAPI_EXCHANGE_NAME_WEBSOCKET_HUOBI_CHANNEL_MARKET_DEPTH_REGEX))) {
//        wsMessage.type = MarketDataMessage::Type::MARKET_DATA_EVENTS;
...
#endif
#endif  // INCLUDE_CCAPI_CPP_CCAPI_MARKET_DATA_SERVICE_HUOBI_H_
