...
inline lib::error_code make_error_code(error::value e) { return lib::error_code(static_cast<int>(e), get_category()); }
...
// template<> struct is_error_code_enum_workaround
 ...
 public:
  ...
  lib::error_code init(bool is_server, int inflate_bits_override = 0) {
    ...
    if (inflate_bits_override == 0) {
      ...
      ret = inflateInit2(&m_istate, inflate_bits_override
                         //            31
      );
    }
...
#endif  // WEBSOCKETPP_DECOMPRESS_WORKAROUND_HPP
