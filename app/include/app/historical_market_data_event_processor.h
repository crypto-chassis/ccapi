...
class HistoricalMarketDataEventProcessor {
 public:
   HistoricalMarketDataEventProcessor(AppLogger* appLogger,std::function<bool(const Event& event)> eventHandler):appLogger(appLogger),eventHandler(eventHandler){
   }
   void processEvent(){
     int clockSeconds = 0;
     ...
     while(currentDateTp<this->endDateTp){
       ...
       std::string fileNameWithDirBase=this->historicalMarketDataDirectory+"/"+this->exchange+"__"+this->baseAsset+"-"+this->quoteAsset+"__"+currentDateISO+"__";
