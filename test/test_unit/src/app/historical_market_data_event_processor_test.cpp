...
namespace ccapi {
  ...
  TEST_F(HistoricalMarketDataEventProcessorTest, processEvent) {
    ...
    while(currentDateTp<this->historicalMarketDataEventProcessor->endDateTp){
      ...
      std::string fileNameWithDirBase=this->historicalMarketDataEventProcessor->historicalMarketDataDirectory+"/"+this->historicalMarketDataEventProcessor->exchange+"__"+this->historicalMarketDataEventProcessor->baseAsset+"-"+this->historicalMarketDataEventProcessor->quoteAsset+"__"+currentDateISO+"__";
    ...
    for (int i = 0; i < expectedMessageList.size(); ++i) {
      ...
      for (int j = 0; j < exptectedElementList.size(); ++j) {
        ...
        for( auto it1 = nameValueMap.cbegin(), end1 = nameValueMap.cend(),
                it2 = expectedNameValueMap.cbegin(), end2 = expectedNameValueMap.cend();
                it1 != end1 || it2 != end2;)
            {
