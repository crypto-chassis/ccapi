...
class ExecutionManagementServiceGeminiTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    this->service =
        std::make_shared<ExecutionManagementServiceGemini>([](Event&, Queue<Event>*) {}, SessionOptions(), SessionConfigs(), wspp::lib::make_shared<ServiceContext>());
    this->credential = {
...
TEST_F(ExecutionManagementServiceGeminiTest, convertRequestCreateOrder) {
  ...
  EXPECT_EQ(std::string(document["nonce"].GetString()), std::to_string(this->timestamp * 1000LL));
