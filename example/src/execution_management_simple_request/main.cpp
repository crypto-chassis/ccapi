...
int main(int argc, char** argv) {
  ...
  if (mode == "create_order") {
    ...
    Request request(Request::Operation::CREATE_ORDER, "bitfinex", argv[2]);
    request.appendParam({
    ...
    Request request(Request::Operation::CANCEL_ORDER, "bitfinex", argv[2]);
    request.appendParam({
        {"ORDER_ID", argv[3]},
    });
    session.sendRequest(request);
    ...
    Request request(Request::Operation::GET_ORDER, "bitfinex", argv[2]);
    request.appendParam({
        {"ORDER_ID", argv[3]},
    });
    session.sendRequest(request);
    ...
    Request request(Request::Operation::GET_OPEN_ORDERS, "bitfinex", argv[2]);
    session.sendRequest(request);
    ...
    Request request(Request::Operation::CANCEL_OPEN_ORDERS, "bitfinex", argv[2]);
    session.sendRequest(request);
  } else if (mode == "get_account_balances") {
    Request request(Request::Operation::GET_ACCOUNT_BALANCES, "bitfinex");
    session.sendRequest(request);
  }
