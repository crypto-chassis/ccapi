...
#### Embedding Tensorflow models

**Objective**

Load a pre-trained Tensorflow model and make inferences from live market data on the fly.  A toy example is shown under `example/src/tf_simple_model`. This example computes the `book pressure` and `spread`, which are inputs to a Tensorflow model for inference, e.g., prediction. 

**Prerequisites:**

* cppflow2 - https://github.com/serizba/cppflow/tree/cppflow2
Copy `include/cppflow` under the example root directory, i.e. `tf_simple_model`.

* Tensorflow for C - https://www.tensorflow.org/install/lang_c
Install Tensorflow for C on your platform following the tutorial.

**Instructions**

Run `create_model.py` to save a Tensorflow model. This creates a `model` folder under the example root directory.
